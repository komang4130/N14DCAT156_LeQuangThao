...
if (move_uploaded_file($_FILES["file"]["tmp_name"], $target_file)) {
    exec("python steg.py ".basename($_FILES["file"]["name"]));
...
$driveInfo = insertFile($service, $title, $description, $mimeType, "audio/".$file_enc, $folderName, $folderDesc);
...
function insertFile($service, $title, $description, $mimeType, $filename, $folderName, $folderDesc) {
	...
	$sql = $conn->prepare("INSERT INTO song(song_name,link,enc_file) VALUES (?,?,?)");
	$sql->bind_param("sss",$song_name,$link,$file_enc);
	$sql->execute();
	$sql->close();
	$conn->close();
